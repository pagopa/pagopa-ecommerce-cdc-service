...
class EcommerceCDCEventDispatcherServiceTest {
    ...
    @Test
    fun `should handle short email addresses in masking`() {
        val shortEmailCases =
            listOf(
                Document().apply {
                    put("_id", "test1")
        ...
        shortEmailCases.forEach { document ->
            val result = ecommerceCDCEventDispatcherService.dispatchEvent(document)
    ...
    @Test
    fun `should handle invalid email formats in masking`() {
        val invalidEmailCases =
            listOf(
                Document().apply {
                    put("_id", "test1")
                    put("transactionId", TEST_TRANSACTION_ID_1)
        ...
        invalidEmailCases.forEach { document ->
            val result = ecommerceCDCEventDispatcherService.dispatchEvent(document)
