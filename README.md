...
| CUSTOM_LIVENESS_INDICATOR_CDC_INACTIVITY_THRESHOLD_SECONDS  | custom liveness probe CDC inactivity threshold (meant as diff of last processed event instant from now) for which return broken status. negative values disable last processed event check | long              |         |
